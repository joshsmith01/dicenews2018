...
var _fontawesome = __webpack_require__(20);
...
function buildGetUrl() {
    var restscrollId = (0, _jquery2.default)('#restscroll');
    var catId = restscrollId.data('catid');
    var authId = restscrollId.data('authid');
    var cptId = restscrollId.data('cptid');
    var cptCatId = restscrollId.data('cptcatid');
...
function loadMore(url, responseHeaderNext) {
    ...
    _jquery2.default.ajax({
        ...
        success: function success(data, textStatus, request) {
            ...
            var responseHeaderTotalPages = request.getResponseHeader('X-WP-TotalPages');
            var currentPage = getParameterByName('page', url);
...
var confirmStorage = function confirmStorage() {
    if (nextLink.next) {
        sessionStorage.setItem('nextLink', nextLink.next.href);
        return nextLink;
    }
...
function displayData(nextLink) {
    ...
    var data = JSON.parse(nextPosts);
    _jquery2.default.each(data, function (index, value) {
        var published = new Date(Date.parse(value.date));
        ...
        (0, _jquery2.default)('.stories').append("<article class='grid-x content-standard post-" + value.id + " post type-post status-publish format-standard has-post-thumbnail hentry category-ai category-headline tag-a-i tag-artificial-intelligence tag-google'>" + (image ? "<div class='article-excerpt-img'>" + "<a href='" + value.link + "'>" + "<img src='" + image + "'>" + "<div class='overlay'></div>" + "</a>" + "</div>" : "<div class='small-12 cell'>") + "<div class='article-excerpt-details'>" + "<header>" + "<h2 class='entry-title'>" + "<a href='" + value.link + "'>" + value.title.rendered + "</a>" + "</h2>" + "<div class='entry-meta entry-meta-standard'>" + "<p class='byline author'>" + value._embedded.author["0"].name + "</p>" + "<time class='updated' datetime='" + value.date + "'>, " + monthName[published.getMonth(Date.parse(value.date))] + " " + published.getDate(Date.parse(value.date)) + ", " + published.getFullYear(Date.parse(value.date)) + "</time>" + "</div>" + "<div class='entry-meta entry-meta-standard'>" + value.time_to_read + "</div>" + "</header>" + "<div class='entry-content hide-for-small-only'>" + "<a href='" + value.link + "'>" + value.get_excerpt_by_char_count + "</a>" + "</div>" + "</article>");
    });
    loadMore(nextLink);
}
...
var clickable = false;
var topLinks = (0, _jquery2.default)("footer .heading");
var innerLinks = (0, _jquery2.default)("footer .menu");
...
function smallWindow() {
    clickable = true;
    innerLinks.slideUp();
    (0, _jquery2.default)('footer .heading').click(function () {
        (0, _jquery2.default)(this).next().children('footer .menu').slideToggle();
    });
}
...
function bigWindow() {
    ...
    (0, _jquery2.default)('footer .menu').css('display', 'block');
}
...
(0, _jquery2.default)(window).resize(function () {
    if ((0, _jquery2.default)(window).width() < breakpoint && !clickable) {
        smallWindow();
    } // If we jump up to larger size, make footer links static
    else if ((0, _jquery2.default)(window).width() > breakpoint - 1 && clickable) {
            bigWindow();
        }
});
...
(0, _jquery2.default)('.locale-topmenu li a').click(function () {
    var dataId = (0, _jquery2.default)(this).data('id');
    (0, _jquery2.default)('.locale-topmenu li a.active').removeClass('active');
    (0, _jquery2.default)('.locale-submenu.active').removeClass('active').addClass('hidden');
    (0, _jquery2.default)('.locale-submenu li a.active').removeClass('active');
    (0, _jquery2.default)('.locale-submenu').find("[data-parent='" + dataId + "']").first().addClass('active');
    (0, _jquery2.default)(this).addClass('active');
    (0, _jquery2.default)('.locale-submenu.submenu_' + dataId).removeClass('hidden').addClass('active');

    var activeSubmenuItemClass = (0, _jquery2.default)('.locale-submenu.active li a.active').attr('class');
    var activeClass = activeSubmenuItemClass.substring(10).slice(0, -7);

    (0, _jquery2.default)('.top-employers.active').removeClass('active').addClass('hidden');
    (0, _jquery2.default)('.top-employers.employers_' + activeClass).removeClass('hidden').addClass('active');
});

// Add view the top lists by menu click. -JMS
(0, _jquery2.default)('.locale-submenu li a').click(function () {
    (0, _jquery2.default)('.locale-menu li a.active').removeClass('active');
    (0, _jquery2.default)('.top-three-posts article.active').removeClass('active').addClass('hidden');

    (0, _jquery2.default)('.locale-topmenu .locale-menu .active').removeClass('active');
    (0, _jquery2.default)('.locale-topmenu .locale-menu .active .locale-submenu').removeClass('active');
    (0, _jquery2.default)(this).addClass('active').parent('.locale-submenu').addClass('active');

    // Analysis articles
    var wholeClass = (0, _jquery2.default)(this).attr('class');
    var activeClass = wholeClass.substring(10).slice(0, -7);

    (0, _jquery2.default)('.article-card' + '.' + activeClass).removeClass('hidden').addClass('active');

    // Display the top employer list
    (0, _jquery2.default)('.top-employers.active').removeClass('active').addClass('hidden');
    (0, _jquery2.default)('.top-employers' + '.employers_' + activeClass).addClass('active').removeClass('hidden');

    // Display hide and show the submenu
    if ((0, _jquery2.default)('.locale-submenu').hasClass('xxxactive')) {
        (0, _jquery2.default)('.locale-submenu.active').removeClass('active').addClass('hidden');
    }
    (0, _jquery2.default)('.submenu_' + activeClass).addClass('active').removeClass('hidden');

    // Update the locale.
    var localeData = (0, _jquery2.default)(".top-employers.active .locale-data").html();
    (0, _jquery2.default)("#dyn-locale-data").text(localeData);

    // Activate the top menu item
    var menuParent = (0, _jquery2.default)(this).data('parent');
    (0, _jquery2.default)('.locale-topmenu li a[data-id="' + menuParent + '"]').addClass('active');
});
