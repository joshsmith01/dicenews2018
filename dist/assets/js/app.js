...
function displayData(nextLink) {
    ...
    _jquery2.default.each(data, function (index, value) {
        ...
        (0, _jquery2.default)('.stories').append("<article class='grid-x content-standard post-" + value.id + " post type-post status-publish format-standard has-post-thumbnail hentry category-ai category-headline tag-a-i tag-artificial-intelligence tag-google'>" + (image ? "<div class='article-excerpt-img'>" + "<img src='" + image + "'>" + "<div class='overlay'></div>" + "</div>" : "<div class='small-12 cell'>") + "<div class='article-excerpt-details'>" + "<header>" + "<h2 class='entry-title'>" + "<a href='" + value.link + "'>" + value.title.rendered + "</a>" + "</h2>" + "<div class='entry-meta entry-meta-standard'>" + "<p class='byline author'>" + value._embedded.author["0"].name + "</p>" + "<time class='updated' datetime='" + value.date + "'>, " + monthName[published.getMonth(Date.parse(value.date))] + " " + published.getDay(Date.parse(value.date)) + ", " + published.getFullYear(Date.parse(value.date)) + "</time>" + "</div>" + "</header>" + "<div class='entry-content hide-for-small-only'>" + value.excerpt.rendered + "</div>" + "</article>");
    });
    loadMore(nextLink);
}
